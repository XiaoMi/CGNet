...
def parse_args():
    ...
    parser.add_argument(
        '--dest_root',
        type=str,
        help='specify the dest path of ann file')
    parser.add_argument(
        '--nproc',
        type=int,
        default=64,
...
def create_av2_infos_mp(root_path,
    ...
    for log_id in log_ids:
        ...
        map_elements['centerline'], map_elements['centerline_graph'] = get_centerline(avm)
...
def get_centerline(avm):
    centerline_list = []

    ids = avm.get_scenario_lane_segment_ids()
    centerline_graph = []
    for id in ids:
        centerline = avm.get_lane_segment_centerline(id)
        centerline_list.append(centerline)
        centerline_graph.append([id, avm.get_lane_segment_successor_ids(id)])
    return centerline_list, centerline_graph
